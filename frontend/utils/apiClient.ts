...
apiClient.interceptors.request.use(
  (config) => {
    ...
    // 打印请求信息
    console.log(`发送请求: ${config.method?.toUpperCase()} ${config.baseURL || ''}${config.url}${config.params ? `?${new URLSearchParams(config.params).toString()}` : ''}`);
...
const processRequest = async (
  ...
  try {
    // 调试信息：输出请求基础信息
    console.log(`[API请求] 方法: ${method}, 原始URL: ${url}, 使用代理: ${USE_PROXY}`);
    ...
    // 服务器返回的错误
    console.error(`[API错误] 服务器响应错误: ${axiosError.response?.status}`, axiosError.response?.data);
 ...
 * @param data 更新数据
 */
export const updateTask = async (taskId: string, data: UpdateTaskRequest): Promise<ApiResponse<any>> => {
