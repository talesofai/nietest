...
import { TaskStatus } from '@/app/api/client/tasks';
...
export async function GET(request: NextRequest) {
    try {
        ...
        console.log('Authorization头部:', headers.get('Authorization') ? '存在' : '不存在');
        console.log('转发请求方法:', 'GET');
        console.log('转发请求头部:', {
            'Content-Type': headers.get('Content-Type'),
            'Accept': headers.get('Accept'),
            'Authorization': headers.has('Authorization') ? `${headers.get('Authorization')?.substring(0, 15)}...` : '无'
        });
        ...
        console.log('后端响应内容:', responseText.substring(0, 100) + (responseText.length > 100 ? '...' : ''));
