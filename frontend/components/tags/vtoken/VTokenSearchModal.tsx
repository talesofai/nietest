...
const VTokenSearchModal: React.FC<VTokenSearchModalProps> = ({ isOpen, onClose, onSelect, type }) => {
    ...
    // 搜索处理函数
    const doSearch = useCallback(async (searchKeyword: string, pageIndex: number) => {
                                    ...
                                    {searchResults.length > 0 ? (
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                                            {searchResults.map((item, index) => (
                                                ...
                                                >
                                                    {/* 图片容器 */}
                                                    <div className="w-full relative overflow-hidden bg-gray-100" style={{ paddingBottom: '133.33%' }}>
                                                        <img
                                                        ...
                                                        {item.heat_score > 0 && (
                                                            <div className="absolute top-1 right-1 bg-black/50 text-white px-1 rounded-sm flex items-center text-xs z-10">
                                                                <HeartFilledIcon className="text-danger mr-1" width={10} height={10} />
                                                                {item.heat_score}
                                                            </div>
                                                        )}
                                                    </div>
